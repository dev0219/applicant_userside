...
router.post('/send_advance_life_email', function (req, res, next) {
  ...
  if(total_data.insurence_want != "ASAP"){
   ...
   currentAddress=total_data.curretMailingAddress;
   mannualAddress=total_data.addressData;
    Object.assign(total_data, {mannualAddress});
    Object.assign(total_data, {currentAddress});
...
function (req, res, next) {
  ...
  form.parse(req, function (err, fields, files){
    ...
    if(total_data.manualAddress == "" || total_data.manualAddress == undefined) {
      if(total_data.address){
        address = total_data.address.address + ", "
        + total_data.address.locality + ", " + total_data.address.administrative_area_level_1;
      }else{
    ...
    Applicant.find({quote_id: total_data.quote_id}, function (err, applicant) {
      ...
      if (total_data.personData.length) {
        ...
        applicant.foundation = total_data.discountsData['foundation'];
        applicant.current_auto_premiun = total_data.current_auto_premium?total_data.current_auto_premium:'';
        applicant.selectedUserType = total_data.selectedUserType;
        applicant.yearBuilt = total_data.yearBuilt;
        applicant.sqft = total_data.sqft;
        applicant.cardatayear = total_data.carData[0]['year']?total_data.carData[0]['year']:'';
        applicant.cartype = total_data.carData[0]['type']?total_data.carData[0]['type']:'';
        applicant.carmodel = total_data.carData[0]['model']?total_data.carData[0]['model']:'';
        applicant.shortrentals = total_data.propertyData['term_rental'];
        applicant.propertyown = total_data.propertyData['property_own'];
        applicant.multipleunit = total_data.propertyData['multiple_unit'];
        applicant.insuranceclaims = total_data.propertyData['insurance_claims'];
        applicant.policycancell = total_data.propertyData['policy_renewed'];
...
router.get("/zapier/agent-login",function(req, res){
  try {
		con.connect(function(err) {
			...
			str = str.replace("(","");
			str = str.replace(")",""); 
			con.query("SELECT * FROM agents WHERE username = '"+str+"' ", function (err, result, fields) {
				if(err) throw err;
				console.log(result);
				res.contentType('json');
				if(result.length){
